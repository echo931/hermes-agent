...
from tools.transcription_cloud import _get_openai_stt_prompt, _transcribe_openai
...
def test_threaded_prompt_takes_precedence_over_prompt_file(monkeypatch, tmp_path):
    ...
    assert _get_openai_stt_prompt(openai_cfg, "hook prompt") == "hook prompt"


def test_missing_prompt_file_fails_open(monkeypatch, tmp_path, caplog):
    missing = tmp_path / "missing.txt"

    assert _get_openai_stt_prompt({"prompt_file": str(missing)}) is None
...
def test_non_utf8_prompt_file_fails_open(tmp_path, caplog):
    ...
    assert _get_openai_stt_prompt({"prompt_file": str(invalid)}) is None
    assert "could not be read" in caplog.text
